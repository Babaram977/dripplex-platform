...
for (const [name, account] of Object.entries(ACCOUNTS)) {
  if (!account.pass) throw new Error(`${name.toUpperCase()}_EMAIL_PASSWORD is required`);
}
...
function buildServer() {
  ...
  server.registerTool('dripplex_list_mail', {
    description: 'List recent DrippleX emails from admin@dripplex.com or support@dripplex.com. Returns metadata only; use get_email for the full message.',
    inputSchema: z.object({
      account: z.enum(['admin', 'support']),
  ...
  server.registerTool('dripplex_get_email', {
    description: 'Read one DrippleX email by IMAP UID. Use after list_mail to inspect a specific message.',
  ...
  server.registerTool('dripplex_send_email', {
    description: 'Send an email from a DrippleX mailbox. Use only after the user has explicitly approved the message.',
    inputSchema: z.object({
      account: z.enum(['admin', 'support']),
      to: z.string().email(),
  ...
  server.registerTool('dripplex_health', {
    description: 'Check connectivity to both DrippleX mailboxes without exposing credentials.',
  }, async () => {
    const result = {};
    for (const account of ['admin', 'support']) {
      try {
        ...
        result[account] = `error: ${error instanceof Error ? error.message : String(error)}`;
      }
    }
...
server.listen(PORT, '0.0.0.0', () => {
  console.error(`[dripplex-mail-bridge] listening on ${PORT}`);
});
