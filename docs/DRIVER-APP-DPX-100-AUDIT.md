...
| Facial verification           | ❌ Missing entirely — no model, no provider integration anywhere in the platform. Not to be confused with the Figma auth flow's "biometric" screen (device Face ID/Touch ID app-unlock via WebAuthn — a different, already-documented gap in Wallet Security)                                          |
...
2. **Slice 2 — Facial verification.** Blocked on the provider decision above. Once
   chosen: schema (`DriverFacialVerification` result record), the provider adapter,
   and wiring into onboarding as a required step before a driver can go `APPROVED`.
