...
> **Amended 2026-09-11 (English-only B1).** §4 and the §19 safety gates below
> carry an amendment: the gate is English only, and a message that cannot be
> confidently classified as English goes to a human and never to Drip AI. This
> copy is the amended one and travels with PR #363, so the implementation and the
> specification agree. It supersedes the copy on `dpx-support-002-phase-2-spec`,
> which should not be merged separately.
...
**AMENDED 2026-09-11 — language coverage.** The original requirement read:
"The gate must be language-aware. DrippleX operates in Nigeria, so coverage must
include at minimum English, Hausa, and Nigerian Pidgin expressions relevant to
money/payment/wallet/safety problems. Language coverage is part of the
deliverable, not an optional refinement." That is withdrawn and replaced by:

> The B1 initial production implementation is **English only**. Messages that
> cannot be confidently classified as English are routed to human handling and
> are **never** passed to Drip AI for automated resolution. Hausa and Nigerian
> Pidgin support is a future language expansion, and is a prerequisite for Drip
> AI handling those languages.

A 93-term Hausa and Pidgin lexicon was written and withdrawn before merge: it
classified "Ina bukatar taimako da odar na" (I need help with my order) as a
physical emergency, along with "I spent a lot of money" and "turn off the light".
A word list nobody qualified has checked is not safety work, and an emergency
queue that most Hausa messages land in cannot tell an emergency from an order
query. Deferring an unreadable message to a person is both safer and simpler.
Full reasoning: `DPX-SUPPORT-002-B1-ENGLISH-ONLY.md`.
...
- English coverage is tested, for ordinary messages as well as genuine
  payment/wallet/safety ones;
- messages that are not confidently English are routed to a human and never
  reach Drip AI (amended 2026-09-11 — see §4);
