...
| Emergency SOS                                                                       | `api.rides.sos({ latitude?, longitude? })` · `api.rides.sosHistory()` — DPX-SAFETY-001. Active trip only; the backend explains when there is none. Never auto-dials emergency services. |
| Share Trip                                                                          | `api.rides.share(id)` → public tracking link (already wired; this row previously said otherwise and was stale)                                                                          |
| Saved Places · Schedule Ride · Promo Code · Referral · OPay Payment                 | **No backend in api.ts** → "Not available yet" (don't fake)                                                                                                                             |
