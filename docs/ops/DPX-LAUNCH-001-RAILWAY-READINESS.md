...
| `TERMII_API_KEY` (`TERMII_SENDER_ID`/`TERMII_BASE_URL` have working defaults)                                                     | backend                                                                    | Real SMS delivery for phone OTP (registration/login) — see `docs/ops/DPX-LAUNCH-005-NOTIFICATION-PROVIDERS.md`                                               | **Yes** — DPX-LAUNCH-004 found this unconfigured means OTP is generated and logged but never actually texted to the user; no real user can complete phone registration/login without it                                                                                                                                                                                                                                                      |
| `RESEND_API_KEY` (`RESEND_FROM_EMAIL` has a working default, needs a verified sending domain)                                     | backend                                                                    | Real email delivery for password reset, email verification, and order/ride lifecycle emails — see `docs/ops/DPX-LAUNCH-005-NOTIFICATION-PROVIDERS.md`        | **Yes** for password reset/email verification specifically; order/ride lifecycle emails are important but not launch-blocking on their own                                                                                                                                                                                                                                                                                                   |
