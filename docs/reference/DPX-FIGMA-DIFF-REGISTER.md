...
| ------------------------------------------------ | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Merchant onboarding & management (all screens)   | Fully live — `merchant.controller.ts`, ~30 SDK methods, DB complete | **Superseded 2026-09-14 — see "Merchant Connect" below.** The 2026-08-08 entry read "No UI built. BLOCKED"; that is no longer true. `apps/super-app/src/app/merchantScreen.tsx` is a live merchant portal (Dashboard, Orders, Products, Store, Earnings, Rooms, Bookings, Payouts, KYC, Bank, Settings) in production use. The Figma gap is unchanged and still open; what changed is that UI was built without it. |
...
### Merchant Connect (POS integration management) — logged 2026-09-14

| Figma                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Backend                                                                                                                                                                                                                                                                                                                                                                                                                              | Action                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **No screen exists.** Not a stub — the 2026-08-07 finding recorded in `dpx-100-figma-screen-mapping.md` §0 stands: _"Merchant has zero screens in the live Figma file."_ `features/MERCHANT/index.ts` is an empty `export {}`, there is no `MERCHANT` entry in `MODULE_GROUPS`, and no merchant key in the `Screen` union. The "Merchant Store" screen under Marketplace is the **customer's** view of a storefront, not merchant-facing management UI. | Live. `GET /integrations` + `/integrations/:id` + `/integrations/:id/test` (MKT-INT-001-C), `GET /integrations/catalogue/products/:integrationId` (#391), `GET /integrations/conflicts/:integrationId` and `PATCH /integrations/conflicts/:conflictId/acknowledge` (#390). Publishing reuses `POST /merchant/products/:id/publish`. All behind `integrations:read`/`integrations:write` **and** `MerchantModuleEnabledGuard` (#392). | **Built without Figma, under explicit founder ruling (2026-09-14)** — recorded here as a Category-C gap rather than resolved, because no design exists to reconcile against. This is a deliberate exception to the standing rule, not an oversight: the merchant portal it lives inside was already built the same way. **Open item:** when the founder designs Merchant Connect in Figma, reconcile the built screen against it and log every difference here. Nothing about the built UI should be treated as an approved design. |

**Why this is logged as a gap rather than a resolution.** The standing rule at the top of this
document says integration work connects Figma to backend exactly as each defines it. Here there is
no Figma side at all, so there was nothing to connect to and nothing to violate — but equally,
nothing approved. The screen's layout, labels, and interaction choices are engineering defaults
awaiting design review, and the register is the place that stays true about which is which.

**What was deliberately not invented.** Merchant Connect exposes only what the backend already
rules on: it reads what one integration imported, publishes through the existing merchant publish
endpoint, and acknowledges conflicts (acknowledgement records that a human looked — it remediates
nothing, and the UI says so). It adds no second publish path, no conflict remedy, no credential
issuance flow, and no bulk catalogue mutation.
