...
| -------------------------- | ------: | ---------------: | ---------------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Auth                       |      11 |                8 |                0 | **Updated 2026-08-07** (DPX-PROFILE-KYC-001 Slices 1-2). 8 built & wired: Welcome, Register, OTP, Sign In (unconfirmed whether it matches ReturningLoginScreen's design), Security, Account, **Profile Setup (new)**, **KYC/Identity Verification (new)**. 3 legitimately N/A: Splash, Permissions, Biometric, Auth Summary are client-only or out of `MODULE_GROUPS`. Prior task-tracker entries claiming Slices 4–5 "completed" were inaccurate — actual commits show Slice 4 was "deferred," Slice 5 only shipped Sign In.                                                              |
...
| Account Management          | ✅           | ✅      | ✅          | Identity card is now a tappable entry point into the real Edit Profile screen (was read-only) — see Profile Setup row.                                                                                                                                                                                                                                                                             |
| Profile Setup               | ✅ (adapted) | ✅      | ✅          | **Built 2026-08-07** — `SuperAppAuthEditProfileScreen` (`/account/profile`), real `PATCH /auth/me` + verification-gated `POST /auth/me/phone/change` + `/email/change` (OTP confirm). No `username` field (founder decision). See `docs/DPX-PROFILE-KYC-001-DESIGN.md`.                                                                                                                            |
| KYC / Identity Verification | ✅ (adapted) | ✅      | ✅          | **Built 2026-08-07** — `SuperAppAuthIdentityVerificationScreen` (`/account/identity-verification`), real `CustomerKycService`/`CustomerKycController` (`kyc/me` self-service + `admin/customer-kyc` review), tiered (Level 0/1/2), 7-state lifecycle. Document images are URL-string fields (no upload backend), same constraint `DriverKyc` lives with. See `docs/DPX-PROFILE-KYC-001-DESIGN.md`. |
...
**Resolved 2026-08-07, later same day (DPX-PROFILE-KYC-001/002, verified under
DPX-PROFILE-KYC-003)** — the "no UI component exists anywhere in the repo" finding above is now
stale for **Profile Setup** and **KYC**: both were built the same day, after this audit ran. See
the §1 summary table (Profile Setup / KYC rows) and `DPX-FIGMA-DIFF-REGISTER.md` for the resolved
state, endpoints, and SDK methods. Kept here verbatim rather than edited, per this doc's own rule
of recording what was actually re-verified at each point in time, not silently rewriting a prior
finding.
...
| Profile Setup                    | `PATCH /auth/me` (`auth.controller.ts`, via `ProfileService`) — **resolved 2026-08-07 (DPX-PROFILE-KYC-001)**: not `PATCH /users/me` as this row originally guessed | `sdk.auth.updateProfile`                                               | ✅ Live                                                                    | ✅ Built & wired 2026-08-07 — `SuperAppAuthEditProfileScreen` (`/account/profile`), see §1 summary table above                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| KYC (Identity Verification)      | `kyc/me` (`CustomerKycController`), `admin/customer-kyc` (`AdminCustomerKycController`)                                                                             | `sdk.kyc`, `sdk.adminCustomerKyc`                                      | ✅ Live                                                                    | ✅ Built & wired 2026-08-07 (DPX-PROFILE-KYC-002) — `SuperAppAuthIdentityVerificationScreen` (`/account/identity-verification`), reached from Account Management's "Identity Verification" row (was the ambiguity noted below in §2 — resolved: this is a genuine customer document-KYC flow, standalone from the driver/merchant `DriverKyc` pipeline it was previously confused with).                                                                                                                                                                                                           |
...
| KYC / Identity Verification             | `kyc/me` (`CustomerKycController`), `admin/customer-kyc` (`AdminCustomerKycController`) — **resolved 2026-08-07 (DPX-PROFILE-KYC-002)**: a genuine standalone customer document-KYC flow, not the OTP/magic-link verification system this row originally flagged as the likely (wrong) match | `sdk.kyc`, `sdk.adminCustomerKyc`                                          | ✅ Live — see §1 row above for the built screen                                                                                                              |
| Verification Status                     | Read via `sdk.kyc.getStatus()`'s `levelAccess`/`status` fields (Level 0/1 derived from phone/email verification, Level 2 from `CustomerKyc`) — no separate screen was built for this, folded into the Identity Verification screen itself                                                    | `sdk.kyc`                                                                  | ✅ Live (as part of Identity Verification, not a separate screen)                                                                                            |
