...
async function runTests() {
  ...
  try {
    ...
    const hasCredentials =
      Array.isArray(getRes.data.credentials) && getRes.data.credentials.length > 0;
  ...
  try {
    ...
    recordTest(
      ...
      merchantAIntegrations.every(
        (i) => i.integrationId === integrationIdA1 || i.integrationId === integrationIdA2,
  ...
  try {
    ...
    const isUpdated =
      updateRes.status === 200 && updateRes.data.vendorName === 'Updated Square POS';
