...
type RouteBetween = 'selfPickup' | 'selfDropoff' | 'none';

export interface LiveMapProps {
  /** The driver's own live position, from watchPosition. */
  self?: LiveMapPoint;
  pickup?: LiveMapPoint;
  ...
  fallbackVariant?: 'assigned' | 'arrived' | 'inprogress';
  fallbackProgress?: number;
}

/** Same dark styling as customer-web's LiveMap, kept in sync manually since
 * the two apps don't share a UI package for ride components yet. */
...
/** Fits the camera to every point given. Must render inside <Map>. */
function CameraController({ points, zoom }: { points: LiveMapPoint[]; zoom?: number }): null {
  const map = useMap();
  ...
  React.useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- key is the stable identity for `points`
  }, [map, key]);
 ...
 * SVG (apps/driver-portal/src/components/ride/map-canvas.tsx) on the trip
 * screen. Falls back to MapCanvas itself when
 * NEXT_PUBLIC_GOOGLE_MAPS_API_KEY isn't configured. Mirrors customer-web's
 * LiveMap (apps/customer-web/src/components/ride/live-map.tsx); the two
 * apps don't share a ride UI package yet, so this is kept in sync by hand.
 */
export function LiveMap({
  ...
  const cameraPoints = [self, pickup, dropoff].filter((point): point is LiveMapPoint => !!point);

  const routeEndpoints: [LiveMapPoint, LiveMapPoint] | null =
    routeBetween === 'selfPickup' && self && pickup
      ? [self, pickup]
      : routeBetween === 'selfDropoff' && self && dropoff
        ? [self, dropoff]
        : null;
  ...
  return (
    <div className={className ?? 'h-full w-full'}>
      ...
      >
        ...
        {pickup ? <Marker position={toLatLng(pickup)} icon={PICKUP_ICON} /> : null}
        {dropoff ? <Marker position={toLatLng(dropoff)} icon={DROPOFF_ICON} /> : null}
        {self ? <Marker position={toLatLng(self)} icon={SELF_ICON} /> : null}
      </Map>
    </div>
  );
}
