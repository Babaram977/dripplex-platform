...
final class DriverPresenceOverlay {
   ...
   * `ic_launcher_round`, came out without one.
   *
   * Corrected 2026-09-06. This comment used to end "The launcher icons were
   * regenerated from the same source in the same commit, so the bubble, the
   * home screen and every notification now carry one mark." They were not:
   * every launcher icon still carried the X-less mark, and only this bubble
   * had a dX. It became true when the approved dX artwork
   * (`resources/dripplex-dx-mark.png`) replaced the master and every asset was
   * regenerated from it. The bubble's own PNG is unchanged — it already
   * carried the X and its lighter outline is tuned for the black circle it
   * sits in, so it is not generated by `scripts/generate-icons.mjs`.
   *
