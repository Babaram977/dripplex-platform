...
describe('REFERRAL_PERMISSIONS', () => {
  it('defines requested referral permissions', () => {
    ...
    expect(REFERRAL_PERMISSIONS.RIDER_USE).toBe('rider:referrals:use');
  ...
  // Same reasoning again for riders: three personas, three permissions, three
  // wallets. Sharing any of them would file a reward against the wrong one.
  it('protects the rider code and stats with the rider permission', () => {
    expect(
      Reflect.getMetadata(PERMISSIONS_KEY, RiderReferralsController.prototype.getMyReferral),
    ).toEqual([REFERRAL_PERMISSIONS.RIDER_USE]);
    expect(
      Reflect.getMetadata(PERMISSIONS_KEY, RiderReferralsController.prototype.getStats),
    ).toEqual([REFERRAL_PERMISSIONS.RIDER_USE]);
  });
