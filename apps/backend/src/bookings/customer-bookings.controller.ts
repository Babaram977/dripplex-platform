...
export class CustomerBookingsController {
  ...
  public async list(
  ...
  ): Promise<ApiSuccessResponse<PaginatedResult<CustomerBookingDto>>> {
    ...
    return {
      success: true,
      data: {
        items: items.map(toCustomerBookingDto),
        meta: {
          page,
          limit: pageSize,
          total,
  ...
  public async getOne(
    @CurrentUser() user: AuthenticatedUser,
    @Param('id', ParseUUIDPipe) id: string,
  ): Promise<ApiSuccessResponse<CustomerBookingDto & { customerMessage: string | null }>> {
    const booking = await this.bookings.getCustomerBooking(user.id, id);
    return {
      success: true,
      data: {
        ...toCustomerBookingDto(booking),
