...
export class FleetFinancialService {
  ...
  /** Pays only an independently approved fleet receivable; commission owed by the fleet is never silently netted. */
  public async initiateSettlement(input: { fleetId: string; amount: number; reference: string; narration?: string }): Promise<void> {
    ...
    const bankRows = await this.prisma.$queryRaw<FleetBankRow[]>`SELECT id, fleet_id, bank_name, bank_code, account_name, account_number, currency, is_default, verified_at, provider_recipient_code, provider, created_at FROM fleet_bank_accounts WHERE fleet_id = ${input.fleetId}::uuid AND is_default = true LIMIT 1`;
    const bank = bankRows[0];
    if (!bank || !bank.verified_at) throw new ConflictDomainException('Fleet has no verified default settlement account');
    const existing = await this.prisma.$queryRaw<Array<{ id: string; status: string }>>`SELECT id, status FROM fleet_settlement_transfers WHERE id = ${input.reference}::uuid LIMIT 1`;
    if (existing[0]) {
    ...
    await this.prisma.$executeRaw`INSERT INTO fleet_settlement_transfers (id, fleet_id, bank_account_id, amount, currency, provider, status, created_at, updated_at) VALUES (${input.reference}::uuid, ${input.fleetId}::uuid, ${bank.id}::uuid, ${input.amount}, 'NGN', 'PAYSTACK', 'PENDING', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`;
    const provider = this.providers.find((p) => String(p.provider) === 'PAYSTACK');
    if (!provider) { await this.failSettlement(input.reference, 'Paystack payout provider is not configured'); return; }
    try {
      const result = await provider.initiatePayout({ reference: input.reference, amount: input.amount, currency: 'NGN', bankCode: bank.bank_code, accountNumber: bank.account_number, accountName: bank.account_name, narration: input.narration ?? 'DrippleX fleet settlement' });
    ...
    } catch (error) { await this.failSettlement(input.reference, error instanceof Error ? error.message : 'Fleet settlement failed'); }
  }
  ...
  public async processProviderResult(reference: string, status: 'SUCCESS' | 'FAILED', providerReference?: string | null, reason?: string): Promise<void> {
    ...
    if (!rows[0]) return;
    if (status === 'SUCCESS') { if (rows[0].status === 'PENDING') await this.completeSettlement(reference, providerReference ?? reference); return; }
  ...
  private async completeSettlement(id: string, providerReference: string) { await this.prisma.$executeRaw`UPDATE fleet_settlement_transfers SET status = 'SUCCESS', provider_reference = ${providerReference}, completed_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ${id}::uuid AND status = 'PENDING'`; }
  private async failSettlement(id: string, reason: string) { await this.prisma.$executeRaw`UPDATE fleet_settlement_transfers SET status = 'FAILED', failure_reason = ${reason.slice(0, 500)}, updated_at = CURRENT_TIMESTAMP WHERE id = ${id}::uuid AND status = 'PENDING'`; }
}
