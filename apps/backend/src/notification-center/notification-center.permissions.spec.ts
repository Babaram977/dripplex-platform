...
describe('notification center controller permissions', () => {
  ...
  it('protects merchant read and mutation endpoints, reusing the customer permission pair', () => {
    expect(
      Reflect.getMetadata(PERMISSIONS_KEY, MerchantNotificationsController.prototype.list),
    ).toEqual([NOTIFICATION_CENTER_PERMISSIONS.CUSTOMER_READ]);
    expect(
      Reflect.getMetadata(
        PERMISSIONS_KEY,
        MerchantNotificationsController.prototype.getPreferences,
      ),
    ).toEqual([NOTIFICATION_CENTER_PERMISSIONS.CUSTOMER_READ]);
    expect(
      Reflect.getMetadata(PERMISSIONS_KEY, MerchantNotificationsController.prototype.markRead),
    ).toEqual([NOTIFICATION_CENTER_PERMISSIONS.CUSTOMER_MANAGE]);
    expect(
      Reflect.getMetadata(PERMISSIONS_KEY, MerchantNotificationsController.prototype.markAllRead),
    ).toEqual([NOTIFICATION_CENTER_PERMISSIONS.CUSTOMER_MANAGE]);
    expect(
      Reflect.getMetadata(PERMISSIONS_KEY, MerchantNotificationsController.prototype.delete),
    ).toEqual([NOTIFICATION_CENTER_PERMISSIONS.CUSTOMER_MANAGE]);
    expect(
      Reflect.getMetadata(
        PERMISSIONS_KEY,
        MerchantNotificationsController.prototype.updatePreferences,
      ),
    ).toEqual([NOTIFICATION_CENTER_PERMISSIONS.CUSTOMER_MANAGE]);
  });
