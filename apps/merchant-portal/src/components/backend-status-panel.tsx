...
const INITIAL: DomainProbe[] = [
  ...
  { id: 'business', label: 'Stores / Business', state: 'idle' },
  { id: 'kyc', label: 'Merchant KYC', state: 'idle' },
...
export function BackendStatusPanel(): React.JSX.Element {
  ...
  const runProbes = React.useCallback(async () => {
    ...
    await run('business', async () => {
      const business = await sdk.merchant.getBusiness();
      return { detail: business.businessName };
    });
    await run('kyc', async () => {
      const kyc = await sdk.merchant.getKycStatus();
      const status = kyc.latest?.verificationStatus ?? kyc.items[0]?.verificationStatus ?? 'none';
      return {
        empty: !kyc.latest && kyc.items.length === 0,
        detail: status,
      };
    });
    await run('wallet', async () => {
      const wallet = await sdk.wallet.merchantWallet();
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="font-display text-3xl font-semibold tracking-tight">Merchant Portal</h1>
          <p className="text-muted-foreground mt-2 text-sm">
            Live Backend Core via <code className="text-xs">src/lib/sdk-merchant.ts</code>
