...
/**
 * DPX-COMMERCIAL-001 Slice 5 — the commercial dashboard: credit-limit policy,
 * a merchant/driver commission-account picker, and manual payment recording.
 * Every read/write here has been an admin-scoped backend endpoint since Slice 1
 * (AdminCommercialCreditSettingsController / AdminCommissionAccountsController).
 *
 * Lives in the Operations Console because this is where the people who use it
 * already are, next to Ride Pricing and Bill Payments. It previously sat in a
 * separate three-route application (`/`, `/login`, `/commercial`) on its own
 * hostname, so changing a credit limit meant a second sign-in to a second
 * portal — and left two "admin" URLs to confuse.
 *
 * Gated on ADMIN_CREDIT_SETTINGS_MANAGE, which `operations_staff` deliberately
 * does NOT hold: editing credit policy is a heavier action than routine ops
 * work, granted to administrator / super_administrator only. The backend
 * enforces that regardless — this gate exists so line staff are not shown a
 * screen that would 403 on them.
 */
export default function AdminCommercialPage(): React.JSX.Element {
  const mayManage = usePermission(COMMERCIAL_MANAGE_PERMISSION);
    ...
    <AppShell>
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
        ...
        {!hydrated ? (
          ...
          // Said plainly rather than rendering cards whose every request would
          // come back 403 — an empty screen full of dashes reads as an outage.
          <EmptyState
            title="You do not have access to commercial settings"
            description="Credit-limit policy and commission accounts are restricted to administrators. Ask an administrator if you need a limit changed or a payment recorded."
          />
        )}
      </div>
    </AppShell>
