...
/**
 * DPX-COMMERCIAL-001 Slice 5 — the console's read surface for the commercial
 * engine (Slices 1-4). Mirrors AdminCommercialCreditSettingsClient /
 * AdminCommissionAccountsClient exactly (both already existed from Slice 1 —
 * nothing new added to the backend). Plain fetch-on-mount hooks, matching the
 * console's existing convention (no react-query dependency in this app).
 *
 * Moved here verbatim from apps/admin-portal, which existed only to host the
 * one screen these serve. The SDK barrel is byte-identical between the two
 * apps, so nothing about the calls changed in the move.
 */
